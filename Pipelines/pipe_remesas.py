...
url = 'https://drive.google.com/u/0/uc?id=1qJK9W8yntSoHrIMIBmpnCb13vjD7_9Gw&export=download'
response = requests.get(url)
data = response.text.encode('utf-8').splitlines()

with beam.Pipeline(options=options) as p:
    pcoll = p | beam.Create(data)
    pcoll | 'Write to GCS' >> beam.io.WriteToText('gs://data-lake-pf/data-lake/remesas.csv')