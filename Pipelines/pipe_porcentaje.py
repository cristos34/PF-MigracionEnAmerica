...
url = 'https://drive.google.com/u/0/uc?id=1lbXepMw19qAVX-qr_9p_gilku9t6er-7&export=download'
...
countries_to_filter = ['Antigua y Barbuda','Argentina','Bahamas','Barbados','Belice','Bolivia','Brasil','Canadá','Cuba','Dominica',
            ...
            'United States','Uruguay','Venezuela','Venezuela, RB','Bahamas, The']

with beam.Pipeline(options=options) as p:
    pcoll = p | beam.Create(data)

    def filter_countries(row, countries_to_filter):
        return row.split(',')[0] in countries_to_filter

    pcoll = pcoll | beam.Filter(filter_countries, countries_to_filter=countries_to_filter)
    pcoll | 'Write to GCS' >> beam.io.WriteToText('gs://data-lake-pf/data-lake/porcentaje.csv')